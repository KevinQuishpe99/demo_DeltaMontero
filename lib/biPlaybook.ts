...
top_up AS (
  SELECT TOP 10 'CRECIERON' AS TIPO, CODIGO, DESCRIPCION, V2024, V2025, DELTA_MONTO
  FROM calc WHERE DELTA_MONTO > 0 ORDER BY DELTA_MONTO DESC
),
top_down AS (
  SELECT TOP 10 'DECRECIERON' AS TIPO, CODIGO, DESCRIPCION, V2024, V2025, DELTA_MONTO
  FROM calc WHERE DELTA_MONTO < 0 ORDER BY DELTA_MONTO ASC
)
SELECT * FROM top_up
UNION ALL
